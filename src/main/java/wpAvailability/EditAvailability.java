...
public class EditAvailability {
	 ...
	 * @param avGraph Verfuegbarkeiten Graph
	 * @param availability Verfuegbarkeiten
	 * @param parent ParentFrame
	 */
	public EditAvailability(final AvailabilityGraph avGraph, Availability availability, JFrame parent) {
		this.availability = availability;
		gui = new EditAvailabilityGUI(this, "Verfügbarkeit bearbeiten", parent);
	 ...
	 * 
	 * @param avGraph Verfuegbarkeits Graph
	 * @param worker Arbeiter
	 * @param day Day Objekt mit Arbeitszeiten
	 * @param parent ParentFrame
	 */
	public EditAvailability(final AvailabilityGraph avGraph, Worker worker, Day day, JFrame parent) {
		...
		try {
			...
			if (availability != null) {
				if (gui.getWorker().equals(AvailabilityGraph.PROJECT_WORKER)) {
					CalendarService.setProjectAvailability(new Availability(gui.getWorker().getLogin(), gui.getAllDay(), gui.getAvailable(), start, end, gui
							.getName(), availability.getId()));
				} else {
					CalendarService.setWorkerAvailability(gui.getWorker().getLogin(),
							new Availability(gui.getWorker().getLogin(), gui.getAllDay(), gui.getAvailable(), start, end, gui.getName(), availability.getId()));
				}
			} else {
				if (gui.getWorker().equals(AvailabilityGraph.PROJECT_WORKER)) {
					CalendarService.setProjectAvailability(new Availability(gui.getWorker().getLogin(), gui.getAllDay(), gui.getAvailable(), start, end, gui
					...
					CalendarService.setWorkerAvailability(gui.getWorker().getLogin(),
							new Availability(gui.getWorker().getLogin(), gui.getAllDay(), gui.getAvailable(), start, end, gui.getName()));
				}

			}
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(gui, "Konnte nicht in DB speichern");
			e.printStackTrace();
		}
	...
	protected ActionListener getSaveListener() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				WPOverview.throwConflict(new Conflict(new Date(System.currentTimeMillis()), Conflict.CHANGED_RESOURCES, WPOverview.getUser().getLogin()));
				save();
			}
		};
	}
