...
public class WPShow {
	...
	/**
	 * Konstruktor
	 * @param over Workpackage GUI
	 * @param selected Ausgewaehltes AP
	 * @param newWp boolean ob neues AP oder nur Modifizieren
	 * @param parent ParentFrame
	 */
	public WPShow(WPOverview over, Workpackage selected, boolean newWp, JFrame parent) {
		...
		if (newWp) {
			...
			this.gui = new WPShowGUI("Neues Arbeitspaket anlegen", this, parent);
			String[] newID = getNextId(selected.getStringID()).split("\\.");
			wp.setLvlIDs(newID);
			wp.setEndDateHope(WpManager.getWorkpackage(wp.getOAPID()).getEndDateHope());
			wp.setStartDateHope(WpManager.getWorkpackage(wp.getOAPID()).getStartDateHope());
			actualWPWorkers = new HashSet<String>();
		} else {
			this.wp = selected;
			this.gui = new WPShowGUI(wp.getStringID() + " | " + wp.getName(), this, parent);
		...
		gui.setValues(wp, getUser().getProjLeiter(), getAufwandArray(), WorkerService.getRealWorkers(), actualWPWorkers);
	...
	/**
	 * prüft, ob ein AP mit der ID bereits in der Datenbank existiert Wird von addWp() und aus WPReassign.setRekPakete() aufgerufen
	 * 
	...
	public boolean newIdIsOK(String newId) {
		...
		if (ids.length != levels) {
			JOptionPane.showMessageDialog(null, "Bitte geben Sie die richtige Anzahl an Ebenen ein \n Ebenenanzahl = " + levels + "!");
			return false;
		}

		if (WpManager.getWorkpackage(newId) != null) {
			JOptionPane.showMessageDialog(null, "Es existiert bereits ein Arbeitspaket mit dieser ID. Bitte geben Sie eine neue ID ein!");
			return false;
		}
		...
		while (actualCheckWp != null && !actualCheckWp.equals(WpManager.getRootAp())) {
			actualCheckWp = WpManager.getWorkpackage(actualCheckWp.getOAPID());
		}
		if (actualCheckWp == null) {
			JOptionPane.showMessageDialog(null, "Es sind keine Oberpakete zu dieser ID vorhanden!");
			return false;
		} else {
			wp.setLvlIDs(ids);
			return true;
	...
	public boolean isSaved() {
		if (saved) {
			...
			JOptionPane.showMessageDialog(gui, "Bitte erst das Arbeitspaket speichern");
			return false;
		}
	}
...
//		if (this.getUser().getProjLeiter()) {
			aufwand = SQLExecuter.executeQuery("SELECT * FROM Aufwand " + "WHERE LVL1ID = " + wp.getLvl1ID() + "and LVL2ID = " + wp.getLvl2ID()
					+ "and LVL3ID = " + wp.getLvl3ID() + "and LVLxID = '" + wp.getLvlxID() + "';");
//		} else {
//			aufwand = SQLExecuter.executeQuery("SELECT * FROM Aufwand " + "WHERE LVL1ID = " + wp.getLvl1ID() + "and LVL2ID = " + wp.getLvl2ID()
//					+ "and LVL3ID = " + wp.getLvl3ID() + "and LVLxID = '" + wp.getLvlxID() + "'" + "and FID_Ma = '" + getUser().getLogin() + "';");
//		}
...
//				}
				row[1] = Controller.DECFORM.format(aufwand.getDouble("Aufwand"));
	...
	public boolean save() {
		...
		if (gui.getIsOAP() && !wp.isIstOAP() && WpManager.calcAC(wp) > 0) {
			JOptionPane.showMessageDialog(gui, "Das Arbeitspaket muss UAP bleiben, da bereits Aufwände eingetragen wurden");
			success = false;
		}

		if (!gui.getIsOAP() && wp.isIstOAP() && WpManager.getUAPs(wp).size() > 0) {
			JOptionPane.showMessageDialog(gui, "Das Arbeitspaket muss OAP bleiben, da UAPs existieren");
			success = false;
		}
	...
	private void checkConflicts() {
		...
		if (newWp) {
			WPOverview.throwConflict(new Conflict(new Date(System.currentTimeMillis()), Conflict.NEW_WP, WPOverview.getUser().getLogin(), wp));
		}

		if (gui.getIsInaktiv() != wp.isIstInaktiv()) {
			WPOverview.throwConflict(new Conflict(new Date(System.currentTimeMillis()), Conflict.CHANGED_ACTIVESTATE, WPOverview.getUser().getLogin(), wp));
		}

		try {
			if (!gui.getBAC().equals(wp.getBac())) {
				WPOverview.throwConflict(new Conflict(new Date(System.currentTimeMillis()), Conflict.CHANGED_BAC, WPOverview.getUser().getLogin(), wp));
			}
			if (gui.getStartHope() != null && !gui.getStartHope().equals(wp.getStartDateHope())) {
				WPOverview.throwConflict(new Conflict(new Date(System.currentTimeMillis()), Conflict.CHANGED_WISHDATES, WPOverview.getUser().getLogin(), wp));
			}
		...
		for (int i = 0; i < wpWorkers.size(); i++) {
			if (guiWorkers.length != wpWorkers.size() || wpWorkers.get(i).equals(guiWorkers[i])) {
				WPOverview.throwConflict(new Conflict(new Date(System.currentTimeMillis()), Conflict.CHANGED_RESOURCES, WPOverview.getUser().getLogin(), wp));
			}
		}
	}
	...
	private boolean saveOAP() {
		...
		try {
			...
			if (wp.equals(WpManager.getRootAp()) && startHope == null) {
				JOptionPane.showMessageDialog(gui, "Das Projekt braucht ein Startdatum");
				save = false;
			}
		} catch (ParseException e) {
			JOptionPane.showMessageDialog(gui, "Datum ist nicht im korrekten Format dd.mm.yyyy");
			save = false;
		}
		...
		if (save) {
			if (endHope != null && startHope != null) {
				if (endHope.before(startHope)) {
					JOptionPane.showMessageDialog(gui, "Gewünschtes Enddatum liegt vor dem gewünschten Startdatum");
					save = false;
				}
			}
		}
		...
		if (save) {
			if (newWp && gui.getIsOAP()) {
				...
				if (WpManager.getRootAp().getLvlIDs().length == wp.getlastRelevantIndex()) {
					JOptionPane.showMessageDialog(gui, "Auf unterster Ebene koennen keine OAP angelegt werden");
		...
		if (save) {
			if (endHope != null && !endHope.equals(wp.getEndDateHope())) {
				if (JOptionPane.showConfirmDialog(gui, "Wollen Sie das geänderte Enddatum in die UAP übernehmen?", "Geändertes Datum",
			...
			if (startHope != null && !startHope.equals(wp.getStartDateHope())) {
				if (JOptionPane.showConfirmDialog(gui, "Wollen Sie das geänderte Startdatum in die UAP übernehmen?", "Geändertes Datum",
						JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
	...
	private boolean saveUAP() {
		...
		Double bac = 0.0;
		double etc = 0, ac = 0, ev, bacKosten = 0, acKosten = 0, eac, etcKosten = 0, wptagessatz;
		...
		wptagessatz = WpManager.calcTagessatz(new ArrayList<String>(actualWPWorkers));
		...
		try {
			...
			if ((bac == null || bac == 0) && save) {
				...
				while (bacString == null) {
					bacString = JOptionPane.showInputDialog(gui, "Bitte BAC eingeben");
				}
		...
		if (newWp || gui.getIsOAP() != wp.isIstOAP()) { // Wenn dieses AP vorher OAP war
			etc = bac;
			...
			try {
				...
				JOptionPane.showMessageDialog(gui, "ETC muss eine Zahl > 0 sein");
				save = false;
			}
		...
		ev = WpManager.calcEV(bacKosten, WpManager.calcPercentComplete(bac, etc, ac));
		...
		if (save) {
			...
			try {
				...
				JOptionPane.showMessageDialog(gui, "Unbekannter Fehler in der Kostenberechnung");
				save = false;
				e1.printStackTrace();
			}
	...
	protected ActionListener getBtnAddAncestorListener() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (isSaved()) {
					new SequencerGUI(getWorkpackage(), SequencerGUI.MODE_ADD_ANCHESTOR, gui);
				}
			}
		};
	}
	...
	protected ActionListener getBtnAddFollowerListener() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (isSaved()) {
					new SequencerGUI(getWorkpackage(), SequencerGUI.MODE_ADD_FOLLOWER, gui);
				}
			}
		};
	}
	...
	protected ActionListener getBtnEditAncestorListener() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (isSaved()) {
					new SequencerGUI(getWorkpackage(), SequencerGUI.MODE_DELETE_ANCHESTOR, gui);
				}
	...
	protected ActionListener getBtnEditFollowerListener() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (isSaved()) {
					new SequencerGUI(getWorkpackage(), SequencerGUI.MODE_DELETE_FOLLOWER, gui);
				}
	...
	protected ActionListener getBtnAddWorkerListener(final JComboBox<Worker> cobAddWorker) {
		return new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (((Worker) cobAddWorker.getSelectedItem()) != null) {
					actualWPWorkers.add(((Worker) cobAddWorker.getSelectedItem()).getLogin());
				}
	...
	protected ActionListener getBtnRemoveWorkerListener(final JComboBox<Worker> cobRemoveWorker) {
		return new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (((Worker) cobRemoveWorker.getSelectedItem()) != null) {
					actualWPWorkers.remove(((Worker) cobRemoveWorker.getSelectedItem()).getLogin());
				}
				gui.fillWorkerCombos(actualWPWorkers);
			}
		};
