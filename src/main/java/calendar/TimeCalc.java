...
public class TimeCalc {
    ...
    /**
     * enthaelt Zuordnung Level -> OAPs ohne Vorgaenger (z.B. 2 - (1.2.0.0,
     * 3.1.0.0))
     */
    ...
    public TimeCalc() {
        ...
        for (Workpackage actualWp : allAp) {
            // Das Setzen wird von Workpackage blockiert, falls es in der
            // Vergangenheit liegt
        ...
        // Liste aller UAP ohne Vorgaenger anlegen, fuer jede Ebene Liste mit
        // allen OAPs ohne Vorgaenger anlegen
        init(uapWithoutAncestors, levelOAP);

        // Hilfsdatenstruktur fuer die verbrauchten Arbeitsstunden fuer einen
        // bestimmten Tag
     ...
     *            wird
     * @return das fruehstmoegliche Startdatum des Arbeitspakets unter
     *         beruecksichtigung von Vorgaengern / OAPs
     */
     ...
     */
    public void init(List<Workpackage> uapWithoutAncestors,
            Map<Integer, List<Workpackage>> levelOAP) {
        ...
        for (int i = 1; i <= maxLevels; i++) {
            levelOAP.put(i, new ArrayList<Workpackage>()); // fuer jede Ebene
                                                           // Set anlegen, dass
                                                           // spaeter mit den
                                                           // OAPs dieser Ebene
                                                           // gefuellt wird
        ...
        for (Workpackage actualWp : allWithoutAncestors) {
            if (!actualWp.isIstInaktiv()) {
                actualWp.setStartDateCalc(avManager.getNextWorkDate(WpManager
                        .getRootAp().getStartDateHope())); // da keine
                                                           // Vorgaenger
                                                           // vorhanden, soll
                                                           // ProjektstartHope
                                                           // gelten
                if (actualWp.isIstOAP()) {
                    ...
                    Date startHope =
                            avManager.getNextWorkDate(actualWp
                                    .getStartDateHope());
                    if (actualWp.getStartDateCalc() != null
                        ...
                        fillUAPHopes(actualWp, startHope);
                            ...
                            && actualWp.getStartDateCalc().before(startHope)) {
                        WPOverview.throwConflict(new Conflict(new Date(System
                                .currentTimeMillis()), Conflict.STARTWISH_FAIL,
                                WPOverview.getUser().getId(), actualWp));
                    }
                    ...
                    if (levelOAP.get(actualLevel) == null) {
                        allLevelWps = new ArrayList<Workpackage>();
                        levelOAP.put(actualLevel, allLevelWps); // OAP in seine
                                                                // Ebenenliste
                                                                // aufnehmen
                    }
                    allLevelWps = levelOAP.get(actualLevel);
                    ...
                    uapWithoutAncestors.add(actualWp); // UAP zur Liste der UAPs
                                                       // ohne Vorgaenger
                                                       // aufnehmen
                }
            }
        ...
        // Sortieren, um sinnvolle Reihenfolge der Abarbeitung der UAP zu
        // erreichen
     ...
     *            Liste von OAPs einer bestimmten Ebene
     */
    private void calcOAP(Workpackage actualOAP) {
        ...
        Controller
                .showConsoleMessage(
                        "OAP "
                                ...
                                + " wird versucht zu berechnen ====================================",
                        Controller.TIME_CALCULATION_DEBUG);
        Controller.showConsoleMessage(
                "Pruefe UAPs von " + actualOAP.getStringID(),
                Controller.TIME_CALCULATION_DEBUG);
            ...
            if (!actualUAP.isIstInaktiv()) {
                ...
                if (actualUAP.getStartDateCalc() == null) {
                    ...
                    if (actualUAP.getStartDateCalc() != null
                        ...
                        WPOverview.throwConflict(new Conflict(new Date(System
                                .currentTimeMillis()), Conflict.STARTWISH_FAIL,
                                WPOverview.getUser().getId(), actualOAP,
                                actualUAP));
                    }
                }
                if (actualUAP.getStartDateCalc() != null
                    ...
                    if (!actualUAP.isIstOAP()) {
                        Controller.showConsoleMessage(actualUAP.getStringID()
                                + " wird berechnet",
                                Controller.TIME_CALCULATION_DEBUG);
                        calcUAP(actualUAP);
                        Controller.showConsoleMessage(actualUAP.getStringID()
                    ...
                    Controller
                            .showConsoleMessage(
                                    actualUAP.getStringID()
                                            + " kann noch nicht berechnet werden, weil noch UAPs oder Vorgaenger fehlen",
                                    Controller.TIME_CALCULATION_DEBUG);
                }
                if (actualOAP.getEndDateCalc() == null
                        || (actualUAP.getEndDateCalc() != null && actualOAP
                                .getEndDateCalc().before(
                                        actualUAP.getEndDateCalc()))) {
        ...
        if (WpManager.getUAPs(actualOAP).isEmpty()) {
            ...
            Controller.showConsoleMessage(
                    "Enddatum zum OAP " + actualOAP.getStringID()
                            + " existiert nicht, da keine UAPs vorhanden",
                    true, Controller.TIME_CALCULATION_DEBUG);
        } else {
            if (done && actualOAP.getEndDateCalc() != null) {
                actualLevelOAPs.remove(actualOAP);

                Controller.showConsoleMessage(
                        "Die Dauer von " + actualOAP.getStringID()
                                + " ist endgueltig berechnet, es endet am "
                                + actualOAP.getEndDateCalc(),
                        Controller.TIME_CALCULATION_DEBUG);
                Controller.showConsoleMessage("",
                        Controller.TIME_CALCULATION_DEBUG);

                Date ancestorStartNew =
                        avManager.getNextWorkDate(actualOAP.getEndDateCalc());

                // 6 SD Neu setzen
                Controller.showConsoleMessage(
                        "Setzte Enddatum von eventuell vorhandenen Nachfolgern von "
                                + actualOAP.getStringID()
                                + " auf "
                                + Controller.DATE_DAY_TIME
                                        .format(ancestorStartNew),
                        Controller.TIME_CALCULATION_DEBUG);

                List<Workpackage> actualFollowers =
                        new ArrayList<Workpackage>(actualOAP.getFollowers());
                ...
                for (Workpackage actualFollower : actualFollowers) {
                    if (!actualFollower.isIstInaktiv()) {
                        ...
                        if (actualFollower.getEndDateCalc() == null
                            ...
                            Controller
                                    .showConsoleMessage(
                                            "Es wurde ein Nachfolger von "
                                                    + actualOAP.getStringID()
                                                    + " gefunden, der jetzt berechnet werden kann",
                                            Controller.TIME_CALCULATION_DEBUG);
    ...
    private void calcUAP(Workpackage uap) {
        ...
        while (stillWork) {
            ...
            if (actualDay.after(new Day(new Date(System.currentTimeMillis())))) {
                ...
                Map<String, Integer> actualDayRemaining =
                        avManager.getRemainingDayWorks(actualDay, workers);
                possibleWorkToday =
                        avManager.getTogetherRemainingDayWork(actualDay,
                                workers);
                allWorkedToday = 0;
                Controller.showConsoleMessage("  Aktueller Tag: "
                        + Controller.DATE_DAY.format(actualDay)
                        + ", heute koennen zusammen " + possibleWorkToday
                        + " Std. gerabeitet werden" + bac,
                        Controller.TIME_CALCULATION_DEBUG);
                if (possibleWorkToday < bac) { // Es muss den ganzen Tag von
                                               // allen gearbeitet werden um das
                                               // AP abzuschliessen
                    for (String actualWorker : workers) {
                        int workerWorkedToday =
                                actualDayRemaining.get(actualWorker);
                        ...
                        if (!pvs.containsKey(actualDay)) {
                            ...
                            pvs.put(actualDay, pvTillNow + workerWorkedToday
                                    * uap.getWpStundensatz());
                        }
                        Controller.showConsoleMessage("     " + actualWorker
                                + " muss heute " + workerWorkedToday
                                + " Stunden arbeiten",
                                Controller.TIME_CALCULATION_DEBUG);
                    }
                                ...
                                if (actualDayRemaining.get(actualWorker) >= optimalWork) {
                                    ...
                                    Controller
                                            .showConsoleMessage(
                                                    "     "
                                                            + actualWorker
                                                            + " muss heute zusaetzlich "
                                                            + workerRemaining
                                                            + " Stunden arbeiten (weil ein anderer Mitarbeiter weniger Zeit hat)",
                                                    Controller.TIME_CALCULATION_DEBUG);
                                }
                            }
                ...
                if ((int) bac == 0) {
                    double percent =
                            (double) allWorkedToday
                                    / (double) possibleWorkToday;
                    double hoursConsumed =
                            (percent * avManager.getCompleteDayWorks(actualDay)
                                    .get(AvailabilityGraph.PROJECT_WORKER
                                            .getLogin()));
                    avManager.addWork(
                    ...
                    actualDayTime.setTime(avManager.getNextWorkTime(
                            actualDay,
                            avManager.getConsumedDayWork(actualDay)
                                    .get(AvailabilityGraph.PROJECT_WORKER
                                            .getLogin())));
                    stillWork = false;
                    ...
                    Controller
                            .showConsoleMessage(
                                    "Alle Stunden von "
                                            ...
                                            + Controller.DATE_DAY
                                                    .format(actualDay)
                                            + " abgearbeitet, es werden "
                                            + (int) hoursConsumed
                                            + " von "
                                            + possibleWorkToday
                                            + " verfügbaren Stunden an diesem Tag verbraucht",
                                    Controller.TIME_CALCULATION_DEBUG);
                } else {
                ...
                Day nextFriday =
                        new Day(ValuesService.getNextFriday(
                ...
                if (actualDay.equals(new Day(new Date(System
                    ...
                    actualDayCal.setTime(actualDay);
                    pvs.put(nextFriday, ValuesService.getApPv(
                    ...
                    Controller.showConsoleMessage(
                            "Aktueller Tag ist letzter Tag mit PV "
                                    + Controller.DATE_DAY.format(actualDay)
                                    + "PV "
                                    + ValuesService.getApPv(uap.getWpId(),
                                            nextFridayCal),
        ...
        Controller.showConsoleMessage("Das Arbeitspaket " + uap.getStringID()
                + " wurde fertig berechnet, Enddatum "
        ...
        Controller.showConsoleMessage(
                "Das Startdatum eventuell vorhandener Nachfolger von "
                        + uap.getStringID() + " wird auf "
                        + Controller.DATE_DAY_TIME.format(ancestorStartNew)
                        + " gesetzt", Controller.TIME_CALCULATION_DEBUG);

        List<Workpackage> actualFollowers =
                new ArrayList<Workpackage>(uap.getFollowers());
        Collections.sort(actualFollowers, new APEndDateComparator());
        Collections.sort(actualFollowers, new APFollowerComparator());
        Collections.sort(actualFollowers, new APBacComparator());
        for (Workpackage actualFollower : actualFollowers) {
        ...
        for (Day actualDay : days) {
            if (actualDay.after(new Date(System.currentTimeMillis()))) {
                ...
                if (cal.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY) {
                    ...
                    Controller.showConsoleMessage("PV von " + wpID + " ist am "
                            + Controller.DATE_DAY.format(actualDay) + " "
