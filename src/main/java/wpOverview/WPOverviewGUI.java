 ...
 * 
 * @author  Samson von Graevenitz, Daniel Metzler, Andre Paffenholz, Michael Anstatt, Marc-Eric Baumgärtner
 * @version 2.0 - <aktuelles Datum>
 * 
 * GUI zum auswählen der Arbeitspakete des User <br />
 * 
 * Die verwendeten Icons stammen von: http://sublink.ca/icons/sweetieplus/
 * sowie: http://p.yusukekamiyamane.com/
 * 
 * Diagona Icons
 *
...
public class WPOverviewGUI extends JFrame {
	...
	public static ImageIcon newAp = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/newAP.png")));
	public static ImageIcon delAP = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/delAP.png")));
	public static ImageIcon akt = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/aktualisieren.png")));
	public static ImageIcon aktoap = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/OAPaktualisieren.png")));
	public static ImageIcon dbcalc = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/DBberechnen.png")));
	public static ImageIcon schlies = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/schliessen.png")));
	public static ImageIcon pw = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/pw.png")));
	public static ImageIcon abmeld = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/abmeld.png")));
	public static ImageIcon hilfe = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/hilfe.png")));
	public static ImageIcon info = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/info.png")));
	public static ImageIcon aufw = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/aufwand.png")));
	public static ImageIcon reass = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/reassign.png")));
	public static ImageIcon childrenOut = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/down.png")));
	public static ImageIcon childrenIn = new ImageIcon(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/up.png")));
	private ImageIcon conflictIcon;

	protected JTabbedPane tabs;
	protected JButton btnSchliessen;
	protected JMenuItem miAktualisieren, miAbmelden, miBeenden, miHilfe, miInfo, miAP, miChangePW, miDelAp, miImportInitial, miCalcDuration;
	...
	/**
	 * Konstuktor DBOverviewGUI() Main-Frame bekommt den Namen "WP-Overview" zugewiesen es wird das Windows Look and Feel verwendet die verschiedenen Menüs,
	 * Buttons etc. werden initialisiert es wird ein TabbedPane verwendet um die aktiven und inaktiven Arbeitspaket in verschieden Tabs anzuzeigen und jedes zu
	 * TabbedPane wird das GridBagLayout hinzugefügt
	 * 
	 * @param over
	 */
	public WPOverviewGUI(WPOverview over, JFrame parent) {
		...
		treeOffen = new TreePanel(allUserWp, new ArrayList<Workpackage>(WpManager.getUserWpOpen(WPOverview.getUser())), over, parent);
		treeFertig = new TreePanel(allUserWp, new ArrayList<Workpackage>(WpManager.getUserWpFinished(WPOverview.getUser())), over, parent);
		...
		tabs = createTabs(treeAlle, treeOffen, treeFertig, workers, baseline, availability, conflicts, timeline);
	...
	/**
	 * void initialize() Methode initialize zum erstellen des Layouts Gui ist 610 Pixel breit und 500 Pixel lang
	 */
	 ...
	 * @param treeAlle Tree mit allen APs
	 * @param treeOffen Tree mit offenen APs
	 * @param treeFertig Tree mit fertigen APs
	 * @param workers Workerpanel
	 * @param baseline BaselinePanel
	 * @param availability AvailabilityGraphGUI
	 * @param conflicts ConflictTable
	 * @param timeline APCalendarPanel
	 * @return JTabbedPane Komplettes TabMenu
	 */
	private JTabbedPane createTabs(TreePanel treeAlle, TreePanel treeOffen, TreePanel treeFertig, WorkerPanel workers, BaselinePanel baseline, AvailabilityGraphGUI availability,
			final ConflictTable conflicts, APCalendarPanel timeline) {
		...
		if (ConflictService.getAllConflicts().size() == 0) {
			conflictIcon = new ImageIcon((Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/noWarning.png"))));
		} else {
			conflictIcon = new ImageIcon((Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/warning.png"))));
		}
		if (WPOverview.getUser().getProjLeiter()) {
			...
			tabs.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					if (e.getClickCount() == 2) {
						if (tabs.getTitleAt(tabs.getSelectedIndex()).equals(conflictText)) {
							JFrame conflictFrame = new JFrame();
							conflictFrame.add(spConflicts);
							conflictFrame.setSize(800, 600);
							Controller.centerComponent(wpOverviewGUI, conflictFrame);
							conflictFrame.setVisible(true);
							conflictFrame.addWindowListener(new WindowAdapter() {
								@Override
	 ...
	 * @param legende Legende die die Farben erklaert
	 * @param lblStatusbar JLabel
	 * @return
	 */
	 ...
	 * @param component Componente die scrollbar sein soll
	 ...
	 * @param show Boolean ob an oder abgeschaltet werden soll
	 */
	...
	protected void reloadTrees() {	    
		...
		treeOffen.reload();
		treeFertig.setNodes(allUserWp, new ArrayList<Workpackage>(WpManager.getUserWpFinished(WPOverview.getUser())));
		treeFertig.reload();
		follower.reload();
	...
	protected void throwConflict(Conflict conflict) {
		conflictIcon.setImage(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/warning.png")));
	...
	protected void releaseConflicts() {
		conflictIcon.setImage(Toolkit.getDefaultToolkit().getImage(WPOverviewGUI.class.getResource("/_icons/noWarning.png")));
	}
	/**
